...
void ccdfs(unsigned int a[], unsigned int i, float rcsq[], int n)
{
	unsigned int j;

	//
	for(j=0; j<n; ++j)
		if(a[j]==0 && get_overlap(rcsq[4*i+0], rcsq[4*i+1], rcsq[4*i+2], rcsq[4*j+0], rcsq[4*j+1], rcsq[4*j+2])>GL_define_0_3f)
		{
			//
			a[j] = a[i];
...
unsigned int find_connected_components(unsigned int a[], float rcsq[], unsigned int n)
{
	unsigned int i, cc;
	...
	//
    memset( a , 0 , ( n * sizeof(unsigned int) ) );//清空数组
