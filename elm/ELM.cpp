...
void pr(string msg, MatrixXd &T) { cout << msg << endl << T << endl; }
void pr(string msg, VectorXd &T) { cout << msg << endl << T << endl; }
...
cv::Mat face_align(const char *filename) {
    ...
    for (unsigned long j = 0; j < dets.size(); ++j) {
        ...
        // endl;//获得第一个点的坐标,注意第一个点是从0开始的 cout << "pixel
        // position of second part: " << shape.part(1) <<
        // endl;//获得第二个点的坐标
        //自己改写，打印出全部68个点
        // 				for (int i = 1; i < 69; i++)
        // 				{
    ...
    dlib::extract_image_chips(img, dlib::get_face_chip_details(shapes),
                              face_chips);
    dlib::array2d<dlib::rgb_pixel> equ; //图像格式
    dlib::equalize_histogram(face_chips[0], equ);
    // gettimeofday(&end2, NULL);
    // cout<<"Preprocessing time:"<<(difftimeval(&end2,
    // &start2)/1000000)<<'s'<<endl;
...
void ELM_training(MatrixXd feature, MatrixXd *W, MatrixXd *b, MatrixXd *beta) {
    ...
    cout << "Training time:" << (difftimeval(&end, &start) / 1000000) << 's'
         << endl;
}
// ELM testing
void ELM_testing(MatrixXd feature1, MatrixXd *W, MatrixXd *b, MatrixXd *beta) {
    ...
    cout << "Testing time:" << (difftimeval(&end1, &start1) / 1000000) << 's'
         << endl;
}
// calculate accuracy
