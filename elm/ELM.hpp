...
void getFiles_test(string path,
                   vector<string> &files); //从路径中获取测试集图像的路径

void getFaces_test(Mat SrcImage, Mat &trainingImages,
                   vector<int> &trainingLabels); //重载单个Mat
...
void getFaces_test(string filePath, Mat &trainingImages,
                   vector<int> &trainingLabels); //获取测试集图像
...
void init_face_detector_dlib(
    string face_landmark =
        "shape_predictor_68_face_landmarks.dat"); //初始化dlib人脸检测器
MatrixXd
ELM_in_ELM_face_training_matrix_from_files(); //从文件夹中获取训练集矩阵
